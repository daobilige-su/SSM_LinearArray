...
// the current frame are already ORB feature extracted and mathched with existing mappoints. 
// so, using the u,v (and right u (image in the right image frame in stereo, corresponds to depth in RGBD)) coordinate of mappoints in current frame,
// and the world coordinates of these mappoints in Map, optimize the pose of current frame, which is the main task of tracking.
// its return value is number of correspondance (inlier) mappoints in the current frame and Map.
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    // They said "Set MapPoint vertices", TODO, I think this is misleading. The only vertix in poseoptimization is the pose of the 
    // current frame, the vertix of mappoint is no longer vertix in g2o anymore.
    // their custom difined <g2o::EdgeStereoSE3ProjectXYZOnlyPose> edge is equals to putting mappoint as vertixs and setting them as fixed.
    // so from here, it is essentially computing the edges from the current frame (SE3) to mappoints (XYZ)
    const int N = pFrame->N;//number of mappoints in the current frame
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            // Monocular observation, if monocular version is used.
            ...
            else  // Stereo observation, is stereo/RGBD version is used.
            {
                nInitialCorrespondences++;
                pFrame->mvbOutlier[i] = false;
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            if(chi2>chi2Stereo[it])// if chi2 distance is high, mark it as outlier
            {
                pFrame->mvbOutlier[idx]=true;
                e->setLevel(1);
                nBad++;
    ...
    return nInitialCorrespondences-nBad;// returns the final number of correspondance (inliner) of mappoints in the current frame and Map.
}
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, DOA_handler* pDOAHandler)// TODO NEW
{   
    ...
    //(2) Local MapPoints seen in Local KeyFrames
	...
	int nCurrentSSID = pKF->mnCurrentSSID;
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            {                
                ...
                else // Stereo observation
                {
                    ...
                    optimizer.addEdge(e);
                    vpEdgesStereo.push_back(e);// storing edges and vertexes to remember their order. will be used later when get their optimized values.
    ...
    if(pbStopFlag)// TODO what does these flag means?
        if(*pbStopFlag)
            return;
    ///////////////////////////////
    ...
    // (6) Check for inliers/outliers edges (Keyframes to Mappoints), and delete outlier edges (Keyframes - Mappoints observations links)
    // Check inlier observations       
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)// for Mono Edge
    {
        g2o::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
        MapPoint* pMP = vpMapPointEdgeMono[i];
    ...
    for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)// for Stereo Edge
    {
        g2o::EdgeStereoSE3ProjectXYZ* e = vpEdgesStereo[i];
        MapPoint* pMP = vpMapPointEdgeStereo[i];
    ...
    // Get Map Mutex
    // Erasing outlier edges: correspondance of A Keyframe to A Mappoint is errased. 
    // which means for each Keyframe object, the mappoints is deleted from its observed Mappoints list and Vice Versa.
